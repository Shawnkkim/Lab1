...
def booksearch():
    ...
    if db.execute("SELECT * FROM books WHERE {} LIKE :search".format(search_option), {"search":'%'+search+'%'}).rowcount==0:
        return render_template("error.html", message="Book does not exist or Cannot Find")
...
def book(book_isbn):
    ...
    return render_template("bookdetail.html", book=book, reviews=reviews, review_count=review_count, data=data)
...
def reviews(book_isbn):
    ...
    review_count=db.execute("SELECT COUNT(*) FROM reviews WHERE isbn=:isbn", {"isbn":book_isbn}).fetchone()
    reviews=db.execute("SELECT * FROM reviews WHERE isbn=:isbn", {"isbn":book_isbn}).fetchall()
    return render_template("bookdetail.html", book=book, reviews=reviews, review_count=review_count)
